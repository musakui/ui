 ...
 * @template {IDBCursor} Cursor
 * @param {IDBTransaction} tx transaction
 * @param {(s: IDBObjectStore) => IDBRequest<Cursor | null>} getCursor
 */
export async function* runCursor(tx, getCursor) {
	const done = txDone(tx)
	try {
		for await (const cur of cursorIterator(getCursor(txStore(tx)))) {
			yield cur
		}
	} finally {
		await done
